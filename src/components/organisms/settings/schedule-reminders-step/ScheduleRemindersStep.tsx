...
const CELL_PRIMARY =
  'text-[var(--font-size-xs)] font-medium leading-[18px] tracking-[var(--letter-spacing-label)] text-[var(--color-text-primary)]'
const CELL_SECONDARY =
  'text-[var(--font-size-xs)] font-medium leading-[18px] tracking-[var(--letter-spacing-label)] text-[var(--color-neutral-600)]'

/** Schedule & Reminders — Figma ScheduleStep (node 621:645). */
  ...
  const columns: DataTableColumn<ReviewCycleSchedule>[] = [
    {
      key: 'name',
      header: 'Cycle Name',
      render: (row) => <span className={cn(CELL_PRIMARY, 'truncate')}>{row.name}</span>,
    },
    {
      key: 'scope',
      ...
      widthClassName: 'w-[22%]',
      render: (row) => (
        <span className={CELL_SECONDARY}>{ASSESSMENT_SCOPE_LABELS[row.scope]}</span>
      ),
    },
    {
      key: 'frequency',
        ...
        <span className={CELL_SECONDARY}>
          ...
          {row.frequency === 'custom' && row.customDays && row.customDays.length > 0 && (
            <span className="text-[var(--color-neutral-400)]">
              {' '}
              ({row.customDays.map((d) => DAY_SHORT[d]).join(', ')})
            </span>
          )}
        </span>
      ),
    },
    {
      ...
      header: 'Start Date',
      widthClassName: 'w-[14%]',
      render: (row) => <span className={CELL_SECONDARY}>{formatDisplayDate(row.startDate)}</span>,
    },
    {
      key: 'end',
      header: 'End Date',
      widthClassName: 'w-[14%]',
      render: (row) => <span className={CELL_SECONDARY}>{formatDisplayDate(row.endDate)}</span>,
    },
    {
      key: 'actions',
      header: <span className="sr-only">Actions</span>,
      widthClassName: 'w-[40px]',
      align: 'right',
      render: (row) => (
        <KebabMenu
    ...
    <>
      <div className="flex w-full flex-col gap-4">
        ...
        <section
          ...
          className="flex w-full flex-col gap-6 rounded-2xl border border-[var(--color-neutral-100)] bg-[var(--color-surface)] p-6"
        >
          <div className="flex min-h-[38px] items-center justify-between gap-4">
            <div className="flex max-w-[415px] flex-col">
              <h2 className="text-[18px] font-bold leading-5 text-[var(--color-text-primary)]">
                Schedule & Reminders
              </h2>
              <p className="text-[12px] font-medium leading-[18px] tracking-[var(--letter-spacing-label)] text-[var(--color-neutral-600)]">
            ...
            <div className="flex shrink-0 items-center gap-3">
              ...
              <button
                ...
                className="flex h-[35px] w-[145px] shrink-0 items-center justify-center gap-2 rounded-[7px] bg-[var(--color-primary-600)] px-4 py-[6px] text-[14px] font-normal leading-[21px] text-white transition-colors hover:bg-[var(--color-primary-700)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary-500)] focus-visible:ring-offset-2"
              >
                <IconPlus size={12} />
                Add Schedule
              </button>
